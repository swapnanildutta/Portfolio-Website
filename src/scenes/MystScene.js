...
import {
  Material, Color, Box2, Vector3, Matrix3, NormalBlending, Camera, AmbientLight,
  DirectionalLight, Projector, RenderableVertex, RenderableSprite, RenderableFace,
  PointLight, SpriteMaterial, LineBasicMaterial, VertexColors, LineDashedMaterial,
  MeshLambertMaterial, MeshPhongMaterial, FaceColors, UVMapping, SphericalReflectionMapping,
  CompressedTexture, DataTexture, RepeatWrapping, AdditiveBlending, Scene, Sprite,
  MirroredRepeatWrapping, SubtractiveBlending,
} from 'three';
...
function MystScene() {
  ...
  const mouseX = useRef(-windowHalfX.current);
  const mouseY = useRef(0 - windowHalfY);
  const init = useRef();
  const onWindowResize = useRef();
  const onDocumentMouseMove = useRef();
  const onDocumentTouchStart = useRef();
  const onDocumentTouchMove = useRef();
  ...
  useEffect(() => {
    init.current = () => {
      camera.current = new PerspectiveCamera(75, window.innerWidth / window.innerHeight, 1, 10000);
      camera.current.position.z = 1000;
      scene.current = new Scene();
      particles.currnet = new Array();
      ...
      for (let i = 0, ix = 0; ix < 50; ix++) {
        for (let iy = 0; iy < 50; iy++) {
          particle.current = particles.current[i++] = new Sprite(material);
          particle.current.position.x = ix * 100 - ((50 * 100) / 2);
          particle.current.position.z = iy * 100 - ((50 * 100) / 2);
  ...
  useEffect(() => {
    onDocumentMouseMove.current = (event) => {
      mouseX.current = event.clientX * .5 - windowHalfX.current;
      mouseY.current = 0 - windowHalfY.current;
    };
  });

  useEffect(() => {
    onDocumentTouchStart.current = (event) => {
      if (event.touches.length === 1) {
        event.preventDefault();
        mouseX.current = event.touches[0].pageX * .5 - windowHalfX.current;
        mouseY.current = 0 - windowHalfY.current;
      }
    };
  });
  ...
  useEffect(() => {
    onDocumentTouchMove.current = (event) => {
      if (event.touches.length === 1) {
        event.preventDefault();
        mouseX.current = event.touches[0].pageX * .5 - windowHalfX.current;
        mouseY.current = 0 - windowHalfY.current;
  ...
  useEffect(() => {
    if (!prefersReducedMotion) {
      animate.current = () => {
        document.addEventListener('mousemove', onDocumentMouseMove.current).trigger('mousemove');
  ...
  useEffect(() => {
    render.current = function() {
      ...
      camera.current.position.x += (mouseX.current - camera.current.position.x) * .05;
      camera.current.position.y += (-mouseY.current - camera.current.position.y) * .05;
      camera.current.lookAt(scene.current.position);

      for (let i = 0, ix = 0; ix < 50; ix++) {
        for (let iy = 0; iy < 50; iy++) {
          particle.current = particles.current[i++];
          particle.current.position.y = (Math.sin((ix + this.count) * 0.3) * 50) + (Math.sin((iy + this.count) * 0.5) * 50);
          particle.current.scale.x = particle.scale.y = (Math.sin((ix + this.count) * 0.3) + 1) * 4 + (Math.sin((iy + this.count) * 0.5) + 1) * 4;
        }
      }
  ...
  return (
    <MystContainer ref={container} aria-hidden />
  );
}
...
export default React.memo(MystScene);
